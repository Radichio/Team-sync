...
// ========================================
// DRAG AND DROP FOR OPTIMIZE MODULE
// ========================================

let draggedElement = null;

/**
 * Initialize drag and drop for optimize view
 */
function initializeOptimizeDragDrop() {
    const members = document.querySelectorAll('#optimizeView .member-item');
    const zones = document.querySelectorAll('#optimizeView .zone-members');
    
    // Add drag events to members
    members.forEach(member => {
        member.addEventListener('dragstart', handleOptimizeDragStart);
        member.addEventListener('dragend', handleOptimizeDragEnd);
    });
...
function handleOptimizeDrop(e) {
    ...
    if (draggedElement && e.currentTarget.classList.contains('zone-members')) {
        // Move the member to the new zone
        e.currentTarget.appendChild(draggedElement);
        
        // Recalculate chemistry (placeholder for now)
...
/**
 * Update chemistry display when members are moved
 */
function updateOptimizeChemistry() {
    // Count members in core team
    const coreMembers = document.querySelectorAll('#optimizeCoreMembers .member-item').length;
    const extendedMembers = document.querySelectorAll('#optimizeExtendedMembers .member-item').length;
    
    // Simple chemistry calculation based on team size
    let chemistry = 70;
    if (coreMembers >= 3 && coreMembers <= 5) chemistry = 85;
    if (coreMembers === 4) chemistry = 90;
    if (extendedMembers >= 2) chemistry += 3;
    
    // Update display
    document.getElementById('optimizeChemistryScore').textContent = chemistry;
    
    // Update status
    const statusEl = document.getElementById('optimizeChemistryStatus');
    if (chemistry >= 85) {
        statusEl.textContent = 'Excellent Chemistry';
        statusEl.className = 'chemistry-status-badge excellent';
    } else if (chemistry >= 70) {
        statusEl.textContent = 'Good Chemistry';
        statusEl.className = 'chemistry-status-badge good';
    } else {
        statusEl.textContent = 'Needs Improvement';
        statusEl.className = 'chemistry-status-badge caution';
    }
    
    console.log('Chemistry updated:', chemistry);
}
